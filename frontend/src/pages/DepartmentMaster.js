  ...
  return (
    <div style={{ fontFamily: "'Outfit', sans-serif", paddingBottom: "40px" }}>
      <div style={{ ...card, padding: "28px 32px", marginBottom: "24px", background: "linear-gradient(135deg, rgba(255,184,0,0.1) 0%, rgba(255,61,113,0.08) 100%)", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <div>
          <h2 style={{ margin: 0, fontSize: "24px", fontWeight: 800, color: "var(--text-primary)" }}>Department Master</h2>
          <p style={{ margin: "6px 0 0", color: "var(--text-muted)", fontSize: "14px" }}>Manage your organisation's departments</p>
        </div>
      ...
      <div style={{ ...card, padding: "28px", marginBottom: "24px" }}>
        <h3 style={{ margin: "0 0 20px", color: "var(--text-primary)", fontSize: "16px", fontWeight: 700 }}>Add New Department</h3>
      ...
      <div style={{ ...card, overflow: "hidden" }}>
        <div style={{ padding: "20px 24px", borderBottom: "1px solid var(--border-light)" }}>
          <h3 style={{ margin: 0, color: "var(--text-primary)", fontSize: "16px", fontWeight: 700 }}>All Departments ({departments.length})</h3>
        </div>
        <div style={{ padding: "8px 0" }}>
          {departments.map((dept) => (
            <div key={dept.id} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "16px 24px", borderBottom: "1px solid var(--bg-input)", transition: "background 0.2s" }}
              onMouseEnter={(e) => e.currentTarget.style.background = "var(--bg-hover)"}
              onMouseLeave={(e) => e.currentTarget.style.background = "transparent"}
            >
              <div style={{ display: "flex", alignItems: "center", gap: "14px" }}>
                <div style={{ width: "36px", height: "36px", background: "rgba(255,184,0,0.1)", border: "1px solid rgba(255,184,0,0.2)", borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center", color: "var(--accent-yellow)", fontWeight: 800, fontSize: "14px" }}>
                  {(dept.department_name || dept.name || "?").charAt(0).toUpperCase()}
                </div>
                <span style={{ fontWeight: 700, color: "var(--text-primary)", fontSize: "15px" }}>{dept.department_name || dept.name}</span>
              </div>
              <span style={{ color: "var(--text-muted)", fontSize: "13px", fontWeight: 600 }}>ID #{dept.id}</span>
            </div>
          ))}
          {departments.length === 0 && (
            <div style={{ textAlign: "center", padding: "48px", color: "var(--text-muted)" }}>No departments found. Add one above.</div>
          )}
        </div>
      </div>
    </div>
