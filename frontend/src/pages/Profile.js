...
const formLabelStyle = { display: "block", marginBottom: "8px", fontSize: "13px", fontWeight: 700, color: "#94a3b8" };
const inputStyle = { width: "100%", padding: "14px 16px", background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: "14px", fontSize: "14px", boxSizing: "border-box", outline: "none", color: "#f1f5f9", fontFamily: "'Outfit', sans-serif" };
const detailRowStyle = { display: "flex", justifyContent: "space-between", padding: "16px 0", borderBottom: "1px solid rgba(255,255,255,0.05)" };
const labelStyle = { fontSize: "14px", fontWeight: 700, color: "#94a3b8" };
const valueStyle = { fontSize: "14px", fontWeight: 600, color: "#f1f5f9" };
...
function Profile() {
  ...
  if (loading) return <div style={{ textAlign: "center", color: "#64748b", padding: "60px" }}>Loading profile...</div>;
  if (error) return <div style={{ padding: "40px", color: "#FF3D71" }}>⚠️ {error}</div>;
  ...
  return (
    <div style={{ fontFamily: "'Outfit', sans-serif", paddingBottom: "40px" }}>
      <div style={{ ...card, padding: "28px 32px", marginBottom: "24px", background: "linear-gradient(135deg, rgba(123,97,255,0.12) 0%, rgba(0,184,255,0.08) 100%)", display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: "20px" }}>
        <div>
          <h2 style={{ margin: 0, fontSize: "24px", fontWeight: 800, color: "#f1f5f9" }}>Employee Profile</h2>
          <p style={{ margin: "6px 0 0", color: "#64748b", fontSize: "14px" }}>View and manage your professional details</p>
        </div>
        {!editing && (
      ...
      <div style={{ display: "flex", gap: "24px", flexWrap: "wrap" }}>
        ...
        <div style={{ ...card, padding: "32px", flex: "2", minWidth: "320px" }}>
          <div style={{ display: "flex", flexDirection: "column", alignItems: "center", marginBottom: "32px" }}>
            <div style={{ position: "relative", width: "120px", height: "120px", borderRadius: "50%", overflow: "hidden", border: "2px solid rgba(0,255,194,0.3)", background: "linear-gradient(135deg, #7B61FF 0%, #00B8FF 100%)", display: "flex", justifyContent: "center", alignItems: "center", boxShadow: "0 0 30px rgba(0,255,194,0.2)" }}>
              {profile.profile_pic ? (
              ...
              <label style={{ position: "absolute", bottom: 0, left: 0, right: 0, background: "rgba(0,0,0,0.7)", color: "#00FFC2", textAlign: "center", padding: "6px 0", fontSize: "11px", fontWeight: 800, cursor: "pointer" }}>
          ...
          {!editing ? (
            <div>
              <h3 style={{ margin: "0 0 20px", color: "#f1f5f9", fontSize: "18px", borderBottom: "1px solid rgba(255,255,255,0.05)", paddingBottom: "12px" }}>Personal Details</h3>
            ...
            <form onSubmit={handleSave}>
              <h3 style={{ margin: "0 0 20px", color: "#f1f5f9", fontSize: "18px", borderBottom: "1px solid rgba(255,255,255,0.05)", paddingBottom: "12px" }}>Update Details</h3>
              <div style={{ marginBottom: "20px" }}>
                <label style={formLabelStyle}>Phone Number</label>
                <input type="text" value={phone} onChange={(e) => setPhone(e.target.value)} style={inputStyle} onFocus={e => e.target.style.borderColor = "#00FFC2"} onBlur={e => e.target.style.borderColor = "rgba(255,255,255,0.08)"} />
              </div>
              <div style={{ marginBottom: "24px" }}>
                <label style={formLabelStyle}>Address</label>
                <textarea value={address} onChange={(e) => setAddress(e.target.value)} style={{ ...inputStyle, minHeight: "100px", resize: "vertical" }} onFocus={e => e.target.style.borderColor = "#00FFC2"} onBlur={e => e.target.style.borderColor = "rgba(255,255,255,0.08)"} />
              </div>
                  ...
                  {availableSkills.map((skill) => {
                    ...
                    return (
                      <button type="button" key={skill.id} onClick={() => handleSkillChange(skill.id)} style={{
                        ...
                        background: isSelected ? "rgba(0,255,194,0.1)" : "rgba(255,255,255,0.02)",
              ...
              <div style={{ display: "flex", gap: "12px" }}>
                <button type="submit" style={{ background: "linear-gradient(135deg, #00FFC2, #00B8FF)", color: "#080B13", border: "none", borderRadius: "50px", padding: "12px 28px", fontSize: "14px", fontWeight: 800, cursor: "pointer" }}>Save Changes</button>
                <button type="button" onClick={() => setEditing(false)} style={{ background: "rgba(255,255,255,0.05)", color: "#f1f5f9", border: "1px solid rgba(255,255,255,0.1)", borderRadius: "50px", padding: "12px 28px", fontSize: "14px", fontWeight: 700, cursor: "pointer" }}>Cancel</button>
              </div>
            </form>
          )}
        </div>
        ...
        <div style={{ flex: "1", minWidth: "280px", display: "flex", flexDirection: "column", gap: "24px" }}>
          <div style={{ ...card, padding: "28px" }}>
            <h3 style={{ margin: "0 0 16px", color: "#f1f5f9", fontSize: "16px" }}>🛠 Skills & Expertise</h3>
            <div style={{ display: "flex", flexWrap: "wrap", gap: "8px" }}>
              ...
              }) : <div style={{ color: "#64748b", fontSize: "13px" }}>No skills listed.</div>}
            </div>
          </div>
          <div style={{ ...card, padding: "28px" }}>
            <h3 style={{ margin: "0 0 16px", color: "#f1f5f9", fontSize: "16px" }}>📁 Uploaded Documents</h3>
            <div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
              {profile.images && profile.images.length > 0 ? profile.images.map((img) => (
                <a key={img.id} href={img.image_url.startsWith("http") ? img.image_url : `http://localhost:5000${img.image_url}`} target="_blank" rel="noreferrer" style={{
                  display: "flex", alignItems: "center", justifyContent: "space-between", padding: "12px", background: "rgba(255,255,255,0.02)", borderRadius: "12px", textDecoration: "none", color: "#f1f5f9", fontSize: "13px", fontWeight: 600, border: "1px solid rgba(255,255,255,0.05)"
                }}>
                  <span>📄 File #{img.id}</span>
                  <span style={{ color: "#00B8FF" }}>View ↗</span>
                </a>
