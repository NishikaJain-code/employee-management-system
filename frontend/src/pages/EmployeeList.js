  ...
  return (
    <div style={{ fontFamily: "'Outfit', sans-serif", paddingBottom: "40px" }}>
      <div style={{ ...card, padding: "28px 32px", marginBottom: "24px", background: "linear-gradient(135deg, rgba(0,255,194,0.1) 0%, rgba(0,184,255,0.08) 100%)", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <div>
          <h2 style={{ margin: 0, fontSize: "24px", fontWeight: 800, color: "var(--text-primary)" }}>Employee Directory</h2>
          <p style={{ margin: "6px 0 0", color: "var(--text-muted)", fontSize: "14px" }}>{employees.length} total employees</p>
        </div>
      ...
      <div style={{ ...card, overflow: "hidden" }}>
        <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "14px" }}>
          ...
          <tbody>
            {employees.map((emp) => (
              <tr key={emp.id} style={{ borderBottom: "1px solid var(--bg-input)" }}
                onMouseEnter={(e) => e.currentTarget.style.background = "var(--bg-hover)"}
                onMouseLeave={(e) => e.currentTarget.style.background = "transparent"}
              >
                <td style={{ padding: "14px 16px", color: "var(--text-muted)", fontWeight: 600 }}>{emp.user_id}</td>
                <td style={{ padding: "14px 16px", fontWeight: 700, color: "var(--text-primary)" }}>{emp.name}</td>
                <td style={{ padding: "14px 16px", color: "var(--text-secondary)" }}>{emp.email}</td>
                <td style={{ padding: "14px 16px", color: "var(--text-secondary)" }}>{emp.department_name || "—"}</td>
                <td style={{ padding: "14px 16px", color: "var(--text-secondary)" }}>{emp.designation || "—"}</td>
                <td style={{ padding: "14px 16px", color: "var(--accent-cyan)", fontWeight: 700 }}>{emp.salary ? `₹${Number(emp.salary).toLocaleString("en-IN")}` : "—"}</td>
                <td style={{ padding: "14px 16px", color: "var(--text-secondary)" }}>{emp.joining_date ? new Date(emp.joining_date).toLocaleDateString() : "—"}</td>
                <td style={{ padding: "14px 16px" }}>
                  <div style={{ display: "flex", flexWrap: "wrap", gap: "4px" }}>
                    {emp.skills && emp.skills.length > 0 ? emp.skills.map((s, i) => (
                      <span key={i} style={{ background: "rgba(123,97,255,0.1)", color: "var(--accent-purple)", border: "1px solid rgba(123,97,255,0.2)", padding: "2px 10px", borderRadius: "50px", fontSize: "11px", fontWeight: 700 }}>{s.skill_name}</span>
